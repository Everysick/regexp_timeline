...
streaming_client.on_inited do
  rest_client.create_direct_message(str_user_id, "start up")
  puts 'Connected!'
end

streaming_client.on_error do |message|
  puts "Error: #{message}"
  rest_client.create_direct_message(str_user_id, "faild down")
end

streaming_client.on_timeline_status do |status|
  if regexps.any? { |r| !r.match(status.text).nil? } &&
      d_regexps.all? { |r| r.match(status.text).nil? } &&
      !status.reply? && !status.retweet?
